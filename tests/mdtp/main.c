...
void test_make_root_node(void) {
    IModule *module = sdk_imodule_create("test", "test", (ABI_SERVER_CORE_FUNCTIONS){0}, 0, 0);

    const ABI_MODULE_MDTP_DATA *data = sdk_mdtp_make_root(
        module, sdk_mdtp_make_container("ram", sdk_mdtp_make_value("use", "12", "gb"), NULL), NULL);
